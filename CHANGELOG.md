...
## v2.0（2026年9月10日・正式リリース）
- 第5条（コピーレフト継承）をSHOULDで確定、第6条（CC BY-SA等との関係）を「参考」の位置づけで確定。GOVERNANCE.mdの「未確定事項」を「確定事項」に更新。
- `v2/AAAPS-v2.md` を草案（DRAFT）から正式版に移行。冒頭の草案注記、各条文の「※仮決定」表記を削除し、バージョンをv2.0として確定。
- 第5条（コピーレフト性、MUST/SHOULD未確定）・第6条（CC BY-SA等との関係）は引き続き継続検討事項として明記した状態で正式版に含める。
- README.md の該当リンク表記から「草案」を削除。
...
- en/AAAPS-v1.md：v1が特定創作者向けである旨を明記、v2への特記を追加、人名表記を「MOTOYE Kenkichi（MOTOI Kenkichi）」に整理、日付更新（内容自体は維持）。
- en/README.md：「character art」を「text-based art / ASCII art (AA)」に変更。
- en/HISTORY.md：日付更新（内容維持）。
- CHANGELOG に本エントリを追加。
